...
jQuery(function($){
  ////////////////////////////////
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var BusinessPointImage = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        business_point_file_frame = wp.media.frames.business_point_file_frame = wp.media({
          button: {
            text: jQuery( this ).data( 'uploader_button_text' )
          },
          state : 'rtam-insert-image',
              states : [
                  new BusinessPointImage()
              ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

        // When an image is selected, run a callback.
        business_point_file_frame.on( 'select', function() {

          var state = business_point_file_frame.state('rtam-insert-image');
        ...
        business_point_file_frame.open();
      });
