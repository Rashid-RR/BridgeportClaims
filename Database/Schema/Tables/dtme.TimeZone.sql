...
GO
ALTER TABLE [dtme].[TimeZone] ADD CONSTRAINT [pkTimeZoneInfo] PRIMARY KEY CLUSTERED  ([TimeZoneID]) WITH (FILLFACTOR=100, DATA_COMPRESSION = ROW) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [idxUqTimeZoneIsDefaultIncludeTimeZoneID] ON [dtme].[TimeZone] ([IsDefault]) INCLUDE ([TimeZoneID]) WHERE ([isDefault]=(1)) WITH (FILLFACTOR=100, DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
