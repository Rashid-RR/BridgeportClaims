...
AS	
	SELECT [p].[ClaimID], [p].[PrescriptionID], p.[RxNumber], p.[DateFilled], p.[LabelName],
		...
		[pn].[CreatedOnUTC] NoteCreatedOn, [pn].[UpdatedOnUTC] NoteUpdatedOn, u.[ID] NoteAuthorID
...
GO
CREATE NONCLUSTERED INDEX [idxvwPrescriptionNoteClaimID] ON [dbo].[vwPrescriptionNote] ([ClaimID]) INCLUDE ([NoteAuthorID]) WITH (FILLFACTOR=90, DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
