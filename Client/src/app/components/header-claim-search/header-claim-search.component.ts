...
export class HeaderClaimSearchComponent implements OnInit, OnDestroy {
    ...
    this.rowSearchHits$ = this.searchCtrl.valueChanges.pipe(
      ...
      switchMap((val: any) => {
        ...
        if (val.length > 1) {
          ...
          const searchResult = this.http.getGlobalSearch(
            val,
            this.selectedType
          );
  ...
  private filterCustomerSearchList(
    val: string
  ): Observable<GlobalSearchResult[]> {
    return this.getSearchHits(val).pipe(
      map(response => {
        if (response instanceof Error) {
          ...
          if (hit) {
            ...
            throw new Error(
              'Could not extract GlobalSearchResult objects from API.'
            );
          }
        }
      })
  ...
  private handleAutoCompleteStyles() {
    ...
    const isAutocompleteOpened$ = merge(
      this.autocompleteOpened$.asObservable(),
      isFirstSearchResultReceived$
    ).pipe(shareReplay(1));

    const isSearchInputExpanded$ = combineLatest([
      isSearchCtrlValueExist$,
      isAutocompleteOpened$
    ]).pipe(
      map(
        ([isSearchCtrlValueExist, autocompleteOpened]) =>
          isSearchCtrlValueExist ||
          (!isSearchCtrlValueExist && autocompleteOpened)
      )
    );

    isFirstSearchResultReceived$.subscribe(
      () => (this.isFirstSearchResultReceived = true)
    );
  ...
  private highlightCustomers(rows: any[]): string[] {
    ...
    if (rows && !(rows instanceof HttpErrorResponse)) {
      return rows.map((x: any) => {
        const wholeString =
          x.firstName + ' ' + x.lastName + ' - ' + x.claimNumber;
        return wholeString.replace(
          regexp,
  ...
  onSearchHit(id) {
    ...
    if (this.currentCustomerId) {
      this.goToClaim(this.currentCustomerId);
    }
  }
}
