...
export class ShortPayService {
    ...
    removeShortpay(id: number = undefined) {
        this.loading = true;
        this.http.removeShortPay({ prescriptionPaymentId: id }).single().map(r => r.json()).subscribe(r => {
            this.loading = false;
            this.toast.success(`Shortpay with id ${id} was successfully removed`);
    ...
    get pageEnd() {
        return this.shortpay.length > 1 ? (this.data.pageSize > this.shortpay.length ? ((this.data.page - 1) * this.data.pageSize) + this.shortpay.length : (this.data.page) * this.data.pageSize) : null;
    }
