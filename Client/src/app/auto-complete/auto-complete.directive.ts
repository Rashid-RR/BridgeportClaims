...
export class AutoCompleteDirective implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  @Input('select-on-blur') selectOnBlur = false;
  @Input('match-formatted') matchFormatted = false;
  @Input('auto-select-first-item') autoSelectFirstItem = false;
  @Input('open-on-focus') openOnFocus = true;
  @Input('re-focus-after-select') reFocusAfterSelect = true;
  ...
  @Input('z-index') zIndex = '1';
  @Input('is-rtl') isRtl = false;
  ...
  ngOnInit(): void {
    ...
    // wrap this element with <div class="-auto-complete">
    this.wrapperEl = document.createElement('div');
    this.wrapperEl.className = '-auto-complete-wrapper';
  ...
  private keydownEventHandler = (evt: any) => {
    if (this.componentRef) {
      const component = <AutoCompleteComponent>this.componentRef.instance;
