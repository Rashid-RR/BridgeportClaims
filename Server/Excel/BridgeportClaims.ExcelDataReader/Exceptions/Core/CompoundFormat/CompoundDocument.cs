...
namespace BridgeportClaims.ExcelDataReader.Exceptions.Core.CompoundFormat
{
    internal class CompoundDocument
    {
        ...
        internal List<uint> GetSectorChain(uint sector, List<uint> sectorTable)
        {
            ...
            while (sector != (uint)Fatmarkers.FatEndOfChain)
            {
                chain.Add(sector);
                sector = GetNextSector(sector, sectorTable);
            }

            TrimSectorChain(chain, Fatmarkers.FatFreeSpace);
            return chain;
        }
        ...
        private List<uint> ReadDifSectorChain(BinaryReader reader)
        {
            ...
            if (Header.DifFirstSector != (uint)Fatmarkers.FatEndOfChain)
            {
                try
                {
            ...
            TrimSectorChain(difSectorChain, Fatmarkers.FatFreeSpace);
            ...
            TrimSectorChain(difSectorChain, Fatmarkers.FatEndOfChain);
        ...
        private List<uint> ReadSectorTable(BinaryReader reader, List<uint> chain)
        {
            ...
            TrimSectorChain(sectorTable, Fatmarkers.FatFreeSpace);
        ...
        private uint GetNextSector(uint sector, List<uint> sectorTable)
        {
            ...
            return (uint)Fatmarkers.FatEndOfChain;
        }
    }
}
