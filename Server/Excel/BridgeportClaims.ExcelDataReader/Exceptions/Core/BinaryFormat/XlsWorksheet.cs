...
*/
        ...
        private bool PushCellValue(object[] cellValues, XlsBiffBlankCell cell, ushort xFormat, List<XlsBiffRecord> additionalRecords)
        {
            ...
            switch (cell.Id)
            {
                ...
                case BIFFRECORDTYPE.LABELSST:
                    var tmp = Workbook.Sst.GetString(((XlsBiffLabelSstCell)cell).SstIndex, Encoding);
                ...
                case BIFFRECORDTYPE.MULRK:
                    ...
                    var lastColumnIndex = rkCell.LastColumnIndex;
                    for (var j = cell.ColumnIndex; j <= lastColumnIndex; j++)
                    {
        ...
        private void ReadWorksheetGlobals()
        {
            ...
            var maxCellColumn = 0;
            var previousBlocksObservedRows = new Dictionary<int, bool>();
            var observedRows = new Dictionary<int, bool>();
            ...
            while (rec != null && !(rec is XlsBiffEof))
            {
                ...
                {
                    foreach (var row in observedRows.Keys)
                    {
